...
class ta_dailyblog_post_tabs_widget extends WP_Widget {
	/* Constructor method */
	function ta_dailyblog_post_tabs_widget() {
        $widget_ops = array( 'description' => __( "Display popular posts, recent posts and comments in tabbed format." , 'ta-dailyblog' ) );
        $this->WP_Widget( 'ta_dailyblog_post_tabs_widget', __( 'TA DailyBlog: Post Tabs Widget', 'ta-dailyblog' ), $widget_ops );
    }
	...
	function widget( $args, $instance ) {
		...
		<div class="tabbed-widget">
			<ul class="nav nav-tabs role="tablist"">
				<li role="presentation" class="active"><a href="#popular" aria-controls="popular" role="tab" data-toggle="tab"><i class="fa fa-fire"></i><?php _e('Popular', 'ta-dailyblog') ?></a></li>
				<li role="presentation"><a href="#recent" aria-controls="recent" role="tab" data-toggle="tab"><i class="fa fa-clock-o"></i><?php _e('Recent', 'ta-dailyblog') ?></a></li>
										...
										<div class="tab-image">
											<?php if ( has_post_thumbnail( $p['post_id'] ) ) : ?>
												<a href="<?php echo $p['post_permalink']; ?>" title="<?php echo $p['post_title']; ?>">
													<?php echo get_the_post_thumbnail( $p['post_id'], 'tab-images' ); ?>
												</a>
											<?php endif; ?>
										</div>

										<div class="post-title">
											<i class="fa fa-eye"></i><i><?php echo $p['views'] . ' ' . __( 'Monthly Views', 'ta-dailyblog' ); ?></i>
								...
								<div class="tab-image">
									<?php if ( has_post_thumbnail() ) : ?>
										<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
											<?php the_post_thumbnail( 'tab-images' ); ?>
										</a>
									<?php endif; ?>
								</div>

								<div class="post-title">
									<i class="fa fa-comments-o"></i><?php comments_popup_link( __( 'Leave a comment', 'ta-dailyblog' ), __( '1 Comment', 'ta-dailyblog' ), __( '% Comments', 'ta-dailyblog' ) ); ?>
				...
				<div role="tabpanel" id="message" class="tab-pane">
					...
					<?php foreach( $recent_comments as $comment ) : ?>
						...
						<div class="post-content">
							...
							<?php if ( $comment->comment_author ) { echo $comment->comment_author; } else { _e( 'Anonymous','ta-dailyblog' ); } ?> <?php _e( 'on','ta-dailyblog' ); ?>
							...
							<p>
								...
								<?php echo strip_tags ( substr( $comment->comment_content, 0, strrpos( substr( $comment->comment_content, 0, 80), ' ' ) ) );?>
								<?php if ( strlen( $comment->comment_content ) > 80 ) { echo '...'; } ?>
	...
	function form( $instance ) {
		...
		<!-- Number of posts -->
		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e('Number of posts to show', 'ta-dailyblog') ?>:</label>
...
}// end ta_dailyblog_post_tabs_widget
